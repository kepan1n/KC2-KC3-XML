...
- per-sheet XML preview для `P + Z` теперь умеет **compare mode**: side-by-side показ `P`/`Z`, source-field chips и отдельный счётчик `derived differences`
- форма реквизитов теперь показывает семантику XML прямо на полях: mini-легенда, фильтр по статусам (`core` / `ИнфПол` / `derived` / `UI-only`) и режим **«Подготовка к выгрузке»**, скрывающий чисто UI-only реквизиты
